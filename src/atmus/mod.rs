...
#[cfg(test)]
mod tests {
    use super::get_pressure;
    use super::atmus;
    use super::get_temperature;
    #[test]
    ...
    #[test]
    fn test_temperature_at_1km() {
        let some_result = get_temperature(1.0);
        assert_eq!(f32::floor(some_result), 281.0);
    }
    #[test]
    fn test_temperature_at_50km() {
        let some_result = get_temperature(50.0);
        assert_eq!(f32::floor(some_result), 270.0);
    }
    #[test]
    fn test_atmus_return_values() {
        let some_result = atmus(1.0);
        assert_eq!(f32::floor(some_result.0), 89874.0);
        assert_eq!(f32::floor(some_result.1), 281.0);
    }
}
